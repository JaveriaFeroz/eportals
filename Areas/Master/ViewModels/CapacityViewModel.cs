using System.ComponentModel.DataAnnotations;

namespace ProcureToPay.Areas.Master.ViewModels
{
    public class CapacityViewModel
    {
        public short? CapacityId { get; set; }

        [Required(ErrorMessage = "Capacity Name is required")]
        [Display(Name = "Capacity Name")]
        [StringLength(100)]
        public string CapacityName { get; set; }

        [Display(Name = "Active")]
        public bool IsActive { get; set; } = true;

        // Read-only audit fields for display
        [Display(Name = "Created By")]
        public string? CreatedBy { get; set; }

        [Display(Name = "Created On")]
        public DateTime? CreatedOn { get; set; }

        [Display(Name = "Updated By")]
        public string? UpdatedBy { get; set; }

        [Display(Name = "Updated On")]
        public DateTime? UpdatedOn { get; set; }
    }

    public class CapacityListViewModel
    {
        public short CapacityId { get; set; }
        public string CapacityName { get; set; }
        public bool IsActive { get; set; }
        public string CreatedBy { get; set; }
        public DateTime CreatedOn { get; set; }
        public string? UpdatedBy { get; set; }
        public DateTime? UpdatedOn { get; set; }
    }

    public class CapacityIndexViewModel
    {
        public List<CapacityListViewModel> Capacities { get; set; } = new List<CapacityListViewModel>();
        public bool ShowInactiveOnly { get; set; } = false;
        public string SearchTerm { get; set; }
    }
    public class CapacityLookupViewModel
    {
        public short CapacityId { get; set; }
        public string CapacityName { get; set; }
    }
}
