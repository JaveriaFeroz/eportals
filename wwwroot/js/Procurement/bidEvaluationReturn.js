// bidEvaluationReturn.js
console.log('Return functionality JavaScript loaded for Bid Evaluation');
...
(function ($) {
    ...
    function getUrl(area, page, action, id) {
        // Assume baseUrl is defined globally in the view, e.g., var baseUrl = '@Url.Content("~/")';
        const baseUrl = window.baseUrl || '/'; // Fallback to root if not defined
    ...
    $(document).ready(function () {
        console.log('✅ Bid Evaluation Return functionality initialization complete');

        returnToSpecificUserModal = $('#returnToSpecificUserModal');

        // Event listener for the "Return" button on the Details page
        // Note: The data attribute on the button must be changed in the Razor view as well.
        $(document).on('click', '#returnButton, .btn-return', function (e) {
            e.preventDefault();
            console.log('Return button clicked');

            // Renamed variable and data attribute lookup
            const bidEvaluationId = $(this).data('bid-evaluation-id');

            if (!bidEvaluationId) {
                console.error('No Bid Evaluation ID found on button.');
                return;
            }

            showReturnModalAndLoadUsers(bidEvaluationId);
        });
        ...
        $(document).on('submit', '#returnForm', function (e) {
            ...
            if (!userId) {
                e.preventDefault();
                showError('Please select a user to return the evaluation to.');
                return false;
            }
     ...
     * @param {number} bidEvaluationId The ID of the bid evaluation.
     */
    function showReturnModalAndLoadUsers(bidEvaluationId) {
        console.log('🎯 Showing return modal for BE ID:', bidEvaluationId);
        ...
        // Renamed controller and action: GetPreviousWorkflowUsers
        const url = getUrl('Procurement', 'BidEvaluation', 'GetPreviousWorkflowUsers');

        // Renamed data key and variable
        const orderData = { bidEvaluationId: bidEvaluationId };

        console.log('🌐 Making AJAX call to:', url);
        console.log('📝 With parameters:', orderData);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data && data.success) {
                    ...
                    // Renamed ID element
                    $('#bidEvaluationId').val(bidEvaluationId);
                } else {
    ...
    function hideLoadingState() {
        ...
        // The dropdown and button state are managed by the change event.
        // We only enable the dropdown here.
        $('#returnToUserId').prop('disabled', false);
        $('#returnComments').prop('disabled', false);
        // The confirm button remains disabled until a user is selected.
    }
